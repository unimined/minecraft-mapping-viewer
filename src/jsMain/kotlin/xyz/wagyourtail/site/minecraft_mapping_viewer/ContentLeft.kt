package xyz.wagyourtail.site.minecraft_mapping_viewer

import io.kvision.core.Display
import io.kvision.core.FlexDirection
import io.kvision.core.Overflow
import io.kvision.core.style
import io.kvision.html.div
import io.kvision.panel.SideTabSize
import io.kvision.panel.TabPanel
import io.kvision.panel.TabPosition
import io.kvision.panel.tab
import io.kvision.table.Table
import io.kvision.table.TableType
import io.kvision.utils.auto
import io.kvision.utils.perc
import io.kvision.utils.px

class ContentLeft(val mainUi: MainUi) : TabPanel(
    tabPosition = TabPosition.LEFT,
    sideTabSize = SideTabSize.SIZE_1,
    className = "content-left"
) {
    init {
        display = Display.GRID
        gridTemplateColumns = "min-content auto"
        padding = 0.px

        style(".tabs-left-container") {
            this@style.setStyle("width", "min-content")
            height = 100.perc
            overflow = Overflow.HIDDEN
        }
        style(".tabs-left-container > ul") {
            height = 100.perc
        }
        style(".tabs-left-content") {
            width = auto
            height = 100.perc
            overflow = Overflow.HIDDEN
        }
        style(".tabs-left-content > div") {
            width = auto
            height = 100.perc
            overflow = Overflow.HIDDEN
        }
    }

    val methods: Table = Table(
        listOf(),
        setOf(TableType.BORDERED, TableType.STRIPED, TableType.HOVER),
        null,
        null,
        null,
        null,
        true,
        null
    ) {
        width = 100.perc
    }

    val methodsTab = tab("Methods") {
        div {
            overflow = Overflow.AUTO
            height = 100.perc

            add(methods)
        }
    }

    val fieldInfo = tab("Field Info") {
        +("todo")
    }

    @Suppress("OVERRIDING_FINAL_MEMBER")
    override fun calculateSideClasses(): Pair<String, String> {
        return "tabs-left-container" to "tabs-left-content"
    }

}